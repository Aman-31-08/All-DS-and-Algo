...
public:
    bool canConstruct(string ransomNote, string magazine) {
        ...
        for(int i=0; i<ransomNote.size(); i++){
            if(mpp[ransomNote[i]] >= 1)    //magazine  have this char of ransomeNote
                mpp[ransomNote[i]]--;   //used once, decrease frequency
            else
                return false;   //No, ransomNote can't be constructed
        }
